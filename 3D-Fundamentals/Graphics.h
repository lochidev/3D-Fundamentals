...
public:
	Graphics(SDL_Renderer* renderer, std::shared_ptr<ZBuffer> zBuf)
		...
		zBuf(zBuf)
	{}
	void DrawTriangle(const Vec3& l, const Vec3& r, const Vec3& h) {
		// find highest in y axis
		const Vec3* p0 = &h;	//highest
		const Vec3* p1 = &l; //left
		const Vec3* p2 = &r; //right
	...
	void DrawFlatTopTriangle(const Vec3& l, const Vec3& r, const Vec3& b)
	{
		...
		for (size_t i = yStart; i < yEnd; i++)
		{
			...
			const auto xStart = std::ceil(lx + l.x - 0.5f);
			const auto rx = sloper * y;
			const auto xEnd = std::ceil(rx + r.x - 0.5f);
			DrawLine({ xStart, float(i), l.z }, { xEnd - 1.0f, float(i), l.z });
		}

	}

	void DrawFlatBottomTriangle(const Vec3& l, const  Vec3& r, const Vec3& h)
	{
		...
		for (size_t i = yStart; i < yEnd; i++)
		{
			...
			const auto lx = slopel * y;
			const auto xStart = std::ceil(lx + h.x - 0.5f);
			const auto rx = sloper * y;
			const auto xEnd = std::ceil(rx + h.x - 0.5f);
			DrawLine({ xStart, float(i) , l.z}, { xEnd - 1.0f, float(i), l.z });
			//SDL_RenderPresent(renderer);
		}
	...
	void DrawCircle(const Vec3& center, int radius)
	{
...
private:
	...
	std::shared_ptr<ZBuffer> zBuf; // always will have a zbuffer cu im lazy
};